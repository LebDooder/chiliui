...
function parse_starttag(tag)
  local elem = {_tag = tag:match("<%s*(%w+)"), _attr = {}}

  for key, _, _val in tag:gmatch("(%w+)%s*=%s*([\"'])(.-)%2", i) do
    local val = unescape(_val)
    if val:match('^%d+$') then -- is Number
      val = tonumber(val)
    elseif val:match('^{.*}$') then -- is Array
      local table = {}
      for var in val:gmatch("[{,]*%s*(.-)[,}]") do
      --  Spring.Echo('[HTML PARSER]    **'..var..'**   [/HTML PARSER]')
        if var:match('function%(.*%).*end') then
          var = loadstring('return '..val:match('function.*end'))()
        end
        table[#table+1] = var
      end
      val = table
